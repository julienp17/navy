...
static game_status_t player_turn(grid_t enemy_grid, pid_t enemy_pid)
{
    ...
    wait_transmission(1);
    my_putstr(coord);
    my_putstr((packet == 1) ? ": hit\n\n" : ": missed\n\n");
    free(coord);
    enemy_grid[row][col] = (packet || enemy_grid[row][col] == HIT) ? HIT : MISS;
    wait_transmission(1);
