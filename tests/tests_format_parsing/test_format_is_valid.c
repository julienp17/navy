...
Test(format_is_valid, incorrect_first_lines)
{
    ...
    fd = open("tests/tests_format_parsing/incorrect_file.txt", O_RDONLY);
    if (fd < 0)
        close(fd);
...
Test(format_is_valid, correct_file1)
{
    ...
    fd = open("tests/tests_format_parsing/correct_file1.txt", O_RDONLY);
    if (fd < 0)
        close(fd);
    for (int i = 0 ; i < 4 ; i++) {
        line = get_next_line(fd);
...
Test(format_is_valid, correct_file2)
{
    ...
    fd = open("tests/tests_format_parsing/correct_file2.txt", O_RDONLY);
    if (fd < 0)
        close(fd);
    for (int i = 0 ; i < 4 ; i++) {
        line = get_next_line(fd);
